...
__version__ = "action.py 2023-12-30T02:19-3:00"
...
#        print(f"Normal hit max_damage={max_damage} extra_damage={extra_damage} damage_roll={damage_roll} remaining_damage={remaining_damage}")
        ...
        # Deliver any remaining damage to the target
        target.current.hit_points -= remaining_damage
        print(f"{target.name} hit self for {remaining_damage} damage leaving {target.current.hit_points} hit points")
    ...
    def do_damage(self, current_time, max_damage):
        ...
        if roll == 20:
            ...
            if threat_roll == 20:
                # Fatal
                saved = actor.makes_save(actor.current.abilities.CON, parent_event.difficulty_class)
                if saved == False:
